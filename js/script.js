const global = {
    ...
    api: {
        apiKey: 'e238ec7dda08ecdd9bb125f14918ea4e',
...
async function displayMovieDetails() {
  ...
  <div class="details-bottom">
    ...
    <div class="list-group">${movie.production_companies.map((company) =>
                `<span>${company.name}</li>`).join(" , ")}</span>
  </div>
</div>
    `
...
function addCommaToNumber(number) {
    // Use a regular expression to add a comma as a thousands separator
...
async function search() {
    ...
    if (global.search.term !== '' && global.search.term !== null) {
        ...
        document.querySelectorAll('#search-term').value = '';
    } else {
...
async function searchAPIData() {
    ...
    const response = await fetch(`${API_URL}/search/${global.search.type}?api_key=${API_KEY}& language=en - US&query=${global.search.term}&${global.search.page}`);
    const data = await response.json();
    hideSpinner();
    return data;
